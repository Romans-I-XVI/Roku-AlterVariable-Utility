Function alterVariable()
    ...
    if (type(msg_remote) = "roInputEvent")
        ...
        print "type = " ; params.var_type
        If params.variable = "" or params.value = "" or params.var_type = ""
        ...
        For i = 0 to variable_array.Count()-1
        	If variable_array[i].Instr("[") <> -1 and variable_array[i].Instr("]") <> -1
        		variable_array[i] = Mid(variable_array[i], 2, variable_array[i].Len()-2)
        		variable_array[i] = variable_array[i].ToInt()
        	End If
        End For
        value = params.value
        var_type = LCase(params.var_type)
        If var_type = "integer"
        	If value.Instr(".") <> -1
        		value = value.ToFloat()
        	Else
